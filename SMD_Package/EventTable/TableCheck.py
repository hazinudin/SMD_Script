...
class EventValidation(object):
    ...
    def coordinate_check(self, routes='ALL', routeid_col="LINKID", long_col="STATO_LONG", lat_col="STATO_LAT",
        ...
        def ref_point_dist(row_obj, max_m, route_df, lrs_geom, start=at_start, from_col=from_m_col, to_col=to_m_col):
            """
            This function will return the distance from inputted point coordinate to its reference point generated by
            its measurement value on the LRS network.
            :param row_obj: The Pandas DataFrame row object.
            :param max_m: The LRS Route maximum measurement value.
            :param route_df: The Route DataFrame, used for inserting measureOnLine Value.
            :param lrs_geom: The route LRS Network geometry object.
            :param start: at_start Boolean parameter.
            :param from_col: The From Measure column of the input table.
            :param to_col: The To Measure column of the input table.
            :return:
            """
            if start:
                # The starting point of a segment in LRS
                if row_obj[lane_code][0] == 'L':
                    point_meas = row_obj[from_col] * 10  # Convert the measurement value to meters (from decimeters)
                elif row_obj[lane_code][0] == 'R':
                    point_meas = row_obj[to_col] * 10
            else:
                # The end point of a segment in LRS
                if row_obj[lane_code][0] == 'L':
                    point_meas = row_obj[to_col] * 10  # Convert the measurement value to meters (from decimeters)
                elif row_obj[lane_code][0] == 'R':
                    point_meas = row_obj[from_col] * 10

            if point_meas / 1000 > max_m:  # If the measurement value is beyond the route max m then pass
                return None
            else:
                lrs_point = lrs_geom.positionAlongLine(point_meas)  # Create a ref point geometry
                dist_to_ref = point_geom.distanceTo(lrs_point)  # The point_geom to ref_point distance
                route_df.loc[index, 'measureOnLine'] = lrs_geom.measureOnLine(point_geom)  # Insert the m-value
